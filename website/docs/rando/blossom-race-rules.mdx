...
<CopyTextComp text="5sQ362kPC_-CfeJ8HaX7cJny-NoYqQZKQc7IwNIwNIwN0u70u70u70u70u70u70u71_qwFLTZK61gbCq2Z80-pphmuHTHbBADknYf_AxydhLmUoxaTnKnI8b6WSbx66LYROE-5GjUYcccP9CNGx20M_b6WAXscqPUjs-umKvfKLWdIa4eJUDhKdJMGxBM69sqHGff3dqD_m" />

- <a
    target="_blank"
    href="/spoilerLogs/Tpr--CriticalMalver_DJ5--SpoilerLog-3iraMNuTg3h.json"
    download
  >
...
- Don't forget that these are excluded: Shop Items, Bulblin Camp, Death Mountain, Lake Lantern Cave
...
- 1 MostSwordsProvince hint
- 1 Miscellaneous hint (a few different possibilities)
  - Barren category, Barren bug gender, Boss Heart Container WotH, Required Item, Basic Item (fallback)
...
- 2 Barren Zone hints
  - <span style={{ 'text-decoration': 'underline' }}>
      Note that we will prioritize hinting a barren dungeon whenever possible!
    </span>
  - Note that a maximum of 1 dungeon can be hinted barren, and it is possible for this hint to be placed on the Ordon sign (as long as Ordon itself is not hinted barren in which case the Ordon sign would hint Ordon as barren).
...
The Barren Zone hints and Path hints will have 3 copies.
...
Always hints on the other hand only have 2 copies each.
...
- Required Item hint
  ...
  - Ex: "They say that the {slingshot} (required) can be found in {Lanayru Province}."
  - Ex: "They say that {Ashei's Sketch} (required) can be found in {Goron Mines}."
- Basic Item hint
  - This is a backup hint that you will only see if other Miscellaneous hints were not possible to generate.
  - This hint is for either a Bow or a Fishing Rod.
  - The item is not guaranteed to be required, and the hint text never indicates if that specific item is required.
    The item text will also be green (for Good) rather than blue (for Required).
  - Ex: "They say that a {bow} can be found in the {Gerudo Desert}."
  - Ex: "They say that a {fishing rod} can be found in {Faron Woods}."
...
- BarrenCategory hint
  ...
  - Ex: "They say that there is nothing to be found behind {Owl Statues}."
- Boss Heart Container WotH hints
  - Whenever a boss heart container has a "Required" CheckStatus, it might get hinted as the starting Miscellaneous hint.
...
- Number of Boomerangs in Dungeons
- Number of Clawshots in Dungeons

Remember that NumItemInArea hints do not direct toward checks, so these hints are generated even if there is already a hint directing toward the check which rewards the boomerang (for example: "Gale Boomerang (required) is in Lanayru Province").

These hints essentially tell you if you should be looking in the Overworld or in Dungeons for boomerangs or clawshots.
These can help with endgame casinos or influence routing decisions earlier into the seed.
For some seeds they might help a lot, and for others they might not be necessary, and that is perfectly okay.

You will randomly get 2 of one hint and 1 of the other.
However, if a dungeon is hinted barren, then you will get exactly 1 of each (the hinted-barren dungeon does not get one of these hints).
...
- If it is possible to hint a dungeon as barren, then we will (max 1).
  - Therefore, if your 3 Barren Zone hints do not include a dungeon, then none of the dungeons are barren.
    This does not imply that they all have hard-required items such as the Boomerang or Spinner though.
