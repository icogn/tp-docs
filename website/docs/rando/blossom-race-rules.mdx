...
- <a
    target="_blank"
    href="/spoilerLogs/Tpr--PlatinumDangoro_96u--SpoilerLog-P303SC-uEKc.json"
    download
  >
...
- We prioritize hinting barren dungeons (no limit).
  If you know all 3 barren zones, then you know exactly which dungeons we were able to hint barren.
  If we did not hint a dungeon as barren, then we were not able to, and it probably has something good in it.
...
Furthermore, the Ordon sign is a special "starting hints" sign which contains your 3 starting hints:
...
- 1 Miscellaneous hint (a few different possibilities)
  - Barren category, Barren bug gender, Boss Heart Container WotH, Required Item, NumBoomerangsInDungeons
...
- 2 Barren Zone hints
  ...
  - If you find all 3 barren zone hints, then you know that any dungeons which were not hinted barren could not be hinted barren.
...
- NumBoomerangsInDungeons hint
  - This tells whether or not the Gale Boomerang is in the Overworld or a Dungeon.
  - Note that this does NOT direct toward the boomerang check.
    It just states a fact without pointing toward anything.
  - Ex: "They say that {0 Gale Boomerangs} can be found in {Dungeons}."
  - Ex: "They say that {1 Gale Boomerang} can be found in a {Dungeon}."
- ItemToItemPath hint
...
- Boss Heart Container WotH hints
  - Whenever a boss heart container has a "Required" CheckStatus, there is a small chance that it gets hinted as the starting Miscellaneous hint.
...
- Number of Clawshots in Dungeons (fills out the remaining dungeon signs)

Remember that NumItemInArea hints do not direct toward checks, so the Clawshot hints are generated even if there is already a hint directing toward a check which rewards a Clawshot.
...
If a dungeon is hinted barren, then it does not get one of these hints.
If all 3 were hinted barren, then none of these hints would be created.
