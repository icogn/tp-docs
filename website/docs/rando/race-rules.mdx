...
Settings string:<br/>
`1sY1M2kv9_yUaP4fqkbqkbqkb2FF_fqUyzN5GVur1WQfJD0hX6G1zddNXot4NKBVANvFS7iku74oe3djDgrXXegP3Vy`
...
- Progressive_Fishing_Rod
  - Makes it so you only have to find a single Progressive_Fishing_Rod to progress in Snowpeak.
  - Also convenient for Upper Zora's River.
...
- Iza Checks
  ...
  - Reasonable players all save these checks until the end.
...
- `Snowpeak Does Not Require Reekfish Scent`
  - You must find the Coral Earring, then get the Reekfish scent in order to access most of Snowpeak.
    - Note that skipping the Reekfish scent with Map Glitch is banned.
  - This makes it so that SPR is never Sphere 0, and you can complete UZR is one pass.
- `City Does Not Require Filled Skybook`
  ...
  - This is an experimental setting, but the idea is to reduce the likelihood that you have to double-dip CitS.
    More research with actual data needs to be done on the difference between this settings and starting with the cannon at Lake Hylia.
...
- What is it:
  - You can skip getting the Reekfish scent by using Map Glitch in Snowpeak.
- Why is it banned:
  - Reduces the item requirements to beat the seed.
