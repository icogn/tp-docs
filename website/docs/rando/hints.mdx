...
- Eldin Province (7 zones)
  ...
  - Kakariko Graveyard
  - Eldin Field
    - Hyrule Field part which is in Eldin Province. Also includes the checks which are on the Bridge of Eldin (1 bug, owl statue checks)
  - North of Eldin Bridge
    - Includes the lava cave (stock cave), the Stalfos grotto which requires Spinner to access, and one of the bugs.
  - Death Mountain
...
- Lanayru Province (9 zones)
  ...
  - Lake Hylia
    - Does not include the Lake Hylia Cave (long lantern cave, hype cave, whatever you want to call it). Plumm is considered a Lake Hylia check.
  - Lake Hylia Cave
...
- Areas which would be considered "Hyrule Field" are split into the following zones:
  - Faron Field
  - Kakariko Gorge
  - Eldin Field (includes the checks on Eldin Bridge)
  - North of Eldin Bridge (stalfos grotto, lava cave, and one bug)
...
Hidden Skills do not prevent a hint zone from being hinted barren, so it is possible for all 7 hidden skills to be in Lake Hylia Cave which is hinted Barren.
You need at least 1 hidden skill to finish the game, so in this case you would have to visit Lake Hylia Cave despite it being hinted Barren.
...
You can only reveal the hints for each location once you fulfill its condition (which is usually standing in a specific location).
...
The website will display an image of the location once the popup shows so it is easier to find the correct location to stand.

The text for this might be:

```
They say that there is {nothing} to be found in {North of Eldin Bridge}.
...
For the `isaac` simulated hints, there are 3 SpoL, 3 Barren, 4 Always, 4 Sometimes, and the AgithaRewards hint is on.
...
The remaining hints are 2 SpoL, 2 Barren, 4 Always, and 4 Sometimes.
...
This means there are effectively 22 signs scattered around the world which have 2 hints each.

So we need to assign 44 hints to signs.

First we create a second copy of the above hints, so instead of 12 (2+2+4+4), we will have 24.

The remaining 20 hints (44 needed - 24 we already have) will be assigned a Random hint (hint telling the contents of a randomly selected check).

The hints are spread out so that you will not find a "sign" with 2 SpoL hints on it for example.
20 of the 22 signs will display one hint which is SpoL/Barren/Always/Sometimes and one hint which is Random, and the other 2 will display 1 SpoL/Barren/Always/Sometimes hint and 1 Sometimes hint.
