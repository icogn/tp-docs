...
Obtain 3 Fused Shadows from dungeon bosses, then race to beat the last boss!

:::tip
Read the sign to the left of Link's house ladder to determine which dungeons are required.
Any unrequired dungeons can be completely ignored (Palace of Twilight is always unrequired).
:::
...
- Access Options
  ...
  - Palace of Twilight is excluded
  - Faron Woods Logic: Open

- Item Pool

  - Everything shuffled with the following exceptions:
    - Poe Souls are Vanilla (can ignore)
    - 8 Agitha Rewards are included (4 male and 4 female)

- Dungeon Items

  - Small Keys: Own Dungeon, but they cannot appear on the boss
  - Big Keys: Keysanity (anywhere)
  - Maps and Compasses: Start With
  - Fused Shadows are only rewarded for defeating required bosses
...
- Excluded checks
  ...
  - Jovani 20 & 60 (can ignore poe souls)
  - Iza 2 ("Iza Raging Rapids Minigame")

- Other
  - Timesavers
    - Everything enabled except "Increase Spinner Speed"
  - Additional Settings
    - The following are on:
      - Transform Anywhere
      - Increase Wallet Capacity
      - Shop Models Show The Replaced Item
  - Dungeon Entrances with special behavior:
    - Arbiter's Does Not Require Bulbin Camp is on
    - City Does Not Require Filled Skybook is on
    - Goron Mines Entrance is Open
    - Temple of Time Entrance is Open
...
You can find one sign per zone, and excluded zones or unrequired dungeons do not contain hints.
...
Edge cases will be explained later.

:::

### Overworld hints

Each hint sign contains 2 hints, and the 2nd hint is a Sometimes hint.
Each hint has 2 copies except Always hints which are split across 3 signs (more on them later).
...
The following hints can be found as the first hint on a sign:
...
- BarrenCategory hint
  ...
  - Ex: "They say that there is {nothing} to be found at the {Lower Desert}."
...
- Boss Heart Container checks
  - Whenever a boss heart container which is not the dungeon reward (so not Stallord) contains a Major Item (preventBarren item), there is a 50% chance that it is hinted.
  - This is because these can lead to some really exciting hints.
  - Ex: "They say that {Goron Mines Fyrus Heart Container} rewards {Lantern}."
...
- Ex: "They say that the {City in the Sky Big Key} can be found in {Temple of Time}."
- Ex: "They say that the {Hyrule Castle Big Key} can be found in {Zora's Domain}."
...
- {Small keys} can be found beyond this point.
  - No checks beyond this point prevent barren, but you can find small keys (small keys do not prevent barren when they are set to Own_Dungeon).
...
- Arbiter's Grounds
  - Hints the status of the 2nd half of the dungeon after the poe gate.
...
We then pick the 3 highest priority checks and use those as A, B, and C.
