...
uint8_5 BITS_TO_RIGHT_MASK [8] =
{
	0x00, 0x01, 0x03, 0x07, 0x0f, 0x1f, 0x3f, 0x7f
};

// bitNumber is one of 0,1,2,3,4,5,6,7
uint8_t count_ones_to_right_of_bit_number (uint8_t byte, uint8_t bitNumber)
{
	if (bitNumber === 0)
	{
		return 0;
	...
	uint8_t maskedByte = byte & BITS_TO_RIGHT_MASK[bitNumber];

	uint8_t result = NIBBLE_LOOKUP[maskedByte >> 4]

	if (bitNumber >= 5)
	{
		result += NIBBLE_LOOKUP[maskedByte & 0x0F];
	}
...
| 0x04   | u16       | maxEnum               | Any enum larger than this is guaranteed to not have any RGB values in the data.      |
| 0x06   | u8 (enum) | dataStructure         | 0x00 means no RGB data (skip all recoloring). 0x01 is simplistic. 0x02 is complex.   |
| 0x07   | u8        | bitTableOffset        | Offset to bitTable section relative to start of header. (Also the header size, 0x10) |
| 0x08   | u16       | cummulativeSumsOffset | Offset to cummulativeSums section relative to start of header.                       |
| 0x0a   | u16       | rgbDataOffset         | Offset to rgbData section relative to start of header.                               |
| 0x0c   | u8[4]     | reserved              | reserved (must all be set to 0x00)                                                   |
...
"CLR0" followed by 0xc bytes which are all 0x00.
