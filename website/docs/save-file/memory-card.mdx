...
You can think of a memory card as a file system. Each file on the memory card is composed of a number of "blocks", and each block is 0x2000 bytes long.
...
The special Directory filesystem block contains a number of Directory Entries which are each 0x40 bytes long. You can think of a DirectoryEntry as metadata about a file on the memory card.
...
You can think of the above as all of the TP data which is on the memory card.
...
CI8 format also means that the 0x200 bytes following the 0xc00 pixel data will be the palette data. Each palette entry is 2 bytes long, meaning a CI8 banner may use up to 0x100 unique palette entries. TP's banner only makes use of the first 0x160 bytes of the palette data space.
...
Block 3 contains the data for the 3 Quest Logs. Block 4 is a backup copy of Block 3, so its data should match Block 3's exactly (assuming nothing unexpected happened during any memory card writes).
...
Each Quest Log's data has the same structure. The structure is rather complex, so it is covered on a different page.
...
In <Link to="#checkcardstat">CheckCardStat</Link>, we determined some things that we can't change without upsetting the game. Ignoring those, we are left with the following:

gameCode, publisherCode, padding_06, upper 5 bits of imageFlags, filename, lastModified, permissions, copyCounter, firstBlockIndex, numBlocks, padding_38

As we already know, the paddings are free real estate.
...
Initially, the imageFlags byte is set to a default value of 0. It is during the 2nd write that TP fills in the 3 important bits.
...
permissions is set to 0x04 (public) during the initial DirectoryEntry creation. For TP, the permissions are never changed after this point. Since only 3 of the bits on the permissions byte carry any meaning, we can do as we please with the other 5 bits as with the imageFlags.

#### copyCounter

This will be incremented when copying between memory cards, so there is not much point in putting anything here.
...
- Any bytes in the filename after the null termination byte will be lost
...
In short, the Header could have been a single block making the game 3 total blocks instead of 4. An animated icon was probably planned at some point but never came to fruition. Below is a detailed explanation.
...
You can see that all of these games indicate that they have a single icon in the `iconFormats` field.
...
Below is a modified memory card which contains TP data which has been adjusted to take only 3 blocks. You can download it and view it in a Gamecube IPL or Dolphin's Memory Card Manager to see that it only uses 3 blocks yet provides the same icon, banner, and comments metadata.
