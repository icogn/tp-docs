...
publisherCode will always be 0x01 which is the publisherCode for Nintendo.

These bytes are available at address 0x80000004 which is where they are copied from when creating the DirectoryEntry.

### padding06

Unused byte which will always be 0xff under normal conditions.

#### Why 0xff?

An formatted memory card's directory blocks will start out as the following:
...
These directory blocks are read into RAM shortly after the game starts up (basically when the WARNING-HEALTH AND SAFETY screen shows). When you tell the game to create TP save data on the memory card, it determines a spot within the directory block data to write the DirectoryEntry before copying it back to the memory card. The padding bytes within the 0x40 byte chunk which is picked to write the TP DirectoryEntry to are never overwritten since there is nothing to put there, so they maintain the value which they had on the memory card.

You can test this by modifying the padding bytes (and adjusting the checksums) before you create TP save data on the memory card. When you create the save data, the padding bytes will be unaffected.

So essentially, their value is unimportant but will always be 0xff under normal circumstances. This also applies to the unused bytes at offset 0x3a of the DirectoryEntry.
...
- A time of 2136/02/07 06:28:15 would give 0xffffffff seconds after 2000/01/01 00:00. I think the only way to get your Gamecube's date this high is to [set it to 2099 and wait 30+ years](https://www.reddit.com/r/Gamecube/comments/kqnhdj/gamecube_calendar_year_past_2099_has_anyone_seen/). Even then, it is possible that the OSTick count returned from OSGetTime is based on a fixed clock and is not adjusted with any changes you make in the IPL (this could be tested). In any case, the clock battery would probably die before this became an issue.
